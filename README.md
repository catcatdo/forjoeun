...
- 선택지 버튼은 제거하고, 지금은 **직접 입력 채팅 중심**으로 진행
- 사용자 입력 후 상대가 타이핑 중처럼 보이는 애니메이션(점점표시) 반영
- 호감도 상승 속도 완화: 선택/채팅 1회당 상승량 완화(미연시 템포 반영)
- 로그인 화면으로 시작 후 닉네임 단위 자동 저장/불러오기
- **Codex 연동(기본 ON)**: 로그인 직후 `채은성` 답장은 기본으로 `/api/reply` AI에 연결되어 생성됨. `/api/reply` 브리지는 localhost Codex 서버에서만 동작

## 로그인/저장 방식
- 닉네임 입력 -> 저장 슬롯 연결
- 같은 닉네임으로 재접속 시 이전 대화 이력 자동 복원
